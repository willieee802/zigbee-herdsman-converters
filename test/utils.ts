...
export function mockDevice(args: {
  modelID: string;
...
function getCluster(ID: string | number) {
  const cluster = Object.entries(Clusters).find((c) =>
    typeof ID === "number" ? c[1].ID === ID : c[0] === ID
  );
...
function mockEndpoint(
  ...
  return {
    ...
    supportsInputCluster: (key) =>
      !!inputClusters.find((ID) => ID === getCluster(key).ID),
    saveClusterAttributeKeyValue: jest
      .fn()
      .mockImplementation(
        (cluster, values) =>
    ...
    save: jest.fn(),
    getClusterAttributeValue: jest
      .fn()
      .mockImplementation(
        (cluster, attribute) => attributes?.[cluster]?.[attribute]
      ),
  };
}
...
export type AssertDefinitionArgs = {
  ...
  configureReporting: {
    [s: number]: [string, ReturnType<typeof reportingItem>[]][];
  };
...
export async function assertDefintion(args: AssertDefinitionArgs) {
  ...
  const expectedToZigbee = definition.toZigbee
    ?.slice(0, definition.toZigbee.length - DefaultTz.length)
    .flatMap((c) => c.key);
  ...
  const expectedExposes = definition.exposes
    ?.map(
      (e) =>
        e.name ??
        `${e.type}${e.endpoint ? "_" + e.endpoint : ""}(${e.features
          ?.map((f) => f.name)
          .join(",")})`
    )
    .sort();
  ...
  for (const endpoint of args.device.endpoints) {
    expect(endpoint.bind).toHaveBeenCalledTimes(
      args.bind[endpoint.ID]?.length ?? 0
    );
    if (args.bind[endpoint.ID]) {
      args.bind[endpoint.ID].forEach((bind, idx) =>
        expect(endpoint.bind).toHaveBeenNthCalledWith(
          idx + 1,
          bind,
          coordinatorEndpoint
        )
    ...
    expect(endpoint.read).toHaveBeenCalledTimes(
      args.read[endpoint.ID]?.length ?? 0
    );
    ...
    expect(endpoint.configureReporting).toHaveBeenCalledTimes(
      args.configureReporting[endpoint.ID]?.length ?? 0
    );
    if (args.configureReporting[endpoint.ID]) {
      args.configureReporting[endpoint.ID].forEach((configureReporting, idx) =>
        expect(endpoint.configureReporting).toHaveBeenNthCalledWith(
          idx + 1,
