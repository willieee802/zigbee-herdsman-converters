...
const fanModes: { [key: string]: number } = {
  low: 2,
  smart: 4,
  medium: 86,
  high: 170,
...
const tzLocal = {
  ...
  vzm36_fan_on_off: {
    ...
    convertSet: async (entity, key, value, meta) => {
      ...
      const state =
        typeof meta.message.fan_state === "string"
 ...
 * fallback to if a transition is not specified by passing 0xffff
 */
const inovelliOnOffConvertSet = async (
    ...
    convert: (model, msg, publish, options, meta) => {
      if (msg.endpoint.ID == 2) {
        if (msg.data.hasOwnProperty("breeze_mode")) {
          const bitmasks = [
            3, 60, 192, 3840, 12288, 245760, 786432, 15728640, 50331648,
            1006632960,
          ];
