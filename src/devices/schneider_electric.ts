      ...
      fromZigbee: [
        {
          ...
          convert: (model, msg, publish, options, meta) => {
            const onOffTransitionTime =
              Number(msg.data["onOffTransitionTime"]) / 10;
            const currentLevel = utils.mapNumberRange(
              Number(msg.data["currentLevel"]),
              0,
              255,
              0,
              100
            ...
            const transition = postfixWithEndpointName(
              "transition",
              msg,
              model,
              meta
            );
            const position = postfixWithEndpointName(
              "position",
              msg,
              model,
              meta
...
const fzLocal = {
  schneider_powertag: {
    ...
    convert: async (model, msg, publish, options, meta) => {
      ...
      if (
        utils.hasAlreadyProcessedMessage(
          msg,
          model,
          msg.data.frameCounter,
          `${msg.device.ieeeAddr}_${commandID}`
        )
      )
        return;
      ...
      if (rxAfterTx) {
        ...
        const networkParameters =
          await msg.device.constructor.adapter.getNetworkParameters();
        const payload = {
          options: 0b000,
...
const definitions: Definition[] = [
  ...
  },
  {
    ...
    fromZigbee: [
      ...
      fz.brightness,
      fz.level_config,
      fz.wiser_lighting_ballast_configuration,
    ],
    toZigbee: [
      tz.light_onoff_brightness,
      tz.level_config,
      tz.ballast_config,
      tz.wiser_dimmer_mode,
    ],
    exposes: [
  ...
  },
  {
    ...
    extend: [
      deviceEndpoints({
        endpoints: { left: 4, right: 3, left_btn: 22, right_btn: 21 },
      }),
  ...
  },
  {
    ...
    configure: async (device, coordinatorEndpoint) => {
      ...
      await reporting.bind(topButtonsEndpoint, coordinatorEndpoint, [
        "genOnOff",
        "genLevelCtrl",
        "genPowerCfg",
      ]);
      ...
      await reporting.bind(bottomButtonsEndpoint, coordinatorEndpoint, [
        "genOnOff",
        "genLevelCtrl",
      ]);
    },
  },
  {
    ...
    exposes: [
      ...
      e.energy(),
      e
        .enum("schneider_pilot_mode", ea.ALL, ["contactor", "pilot"])
    ...
    configure: async (device, coordinatorEndpoint) => {
      ...
      await reporting.instantaneousDemand(endpoint2, {
        min: 0,
        max: 60,
        change: 1,
      });
  ...
  {
    fingerprint: [
      { modelID: "Thermostat", manufacturerName: "Schneider Electric" },
    ],
    model: "CCTFR6400",
    vendor: "Schneider Electric",
    ...
    exposes: [
      ...
      e
        .climate()
        .withSetpoint("occupied_heating_setpoint", 4, 30, 0.5, ea.SET)
        .withLocalTemperature(ea.STATE)
    ...
    configure: async (device, coordinatorEndpoint) => {
      ...
      endpoint1.saveClusterAttributeKeyValue("hvacThermostat", {
        schneiderWiserSpecific: 1,
        systemMode: 4,
        ctrlSeqeOfOper: 2,
      });
      endpoint1.saveClusterAttributeKeyValue("hvacUserInterfaceCfg", {
        keypadLockout: 0,
      });
    },
  },
  ...
  },
  {
    ...
    exposes: [
      e.battery(),
      e
        .binary("keypad_lockout", ea.STATE_SET, "lock1", "unlock")
        .withDescription("Enables/disables physical input on the device"),
      e
        .binary("calibrate_valve", ea.STATE_SET, "calibrate", "idle")
        .withDescription("Calibrates valve on next wakeup"),
      e.enum("valve_calibration_status", ea.STATE, [
        "ongoing",
        "successful",
        "uncalibrated",
        "failed_e1",
    ...
    configure: async (device, coordinatorEndpoint) => {
      ...
      endpoint.saveClusterAttributeKeyValue("hvacThermostat", {
        minHeatSetpointLimit: 7 * 100,
      });
      endpoint.saveClusterAttributeKeyValue("hvacThermostat", {
        maxHeatSetpointLimit: 30 * 100,
      });
      endpoint.saveClusterAttributeKeyValue("hvacThermostat", {
        occupiedHeatingSetpoint: 20 * 100,
      });
      endpoint.saveClusterAttributeKeyValue("hvacThermostat", {
        systemMode: 4,
      });
      // VACT needs binding to endpoint 11 due to some hardcoding in the device
      const coordinatorEndpointB = coordinatorEndpoint
        .getDevice()
        .getEndpoint(11);
  ...
  },
  {
    ...
    exposes: [
      ...
      e.energy(),
      e
        .enum("identify", ea.SET, ["0", "30", "60", "600", "900"])
        .withDescription("Flash green tag for x seconds"),
      e.enum("zone_mode", ea.ALL, [
        "manual",
        "schedule",
        "energy_saver",
        "holiday",
  ...
  },
  {
    ...
    whiteLabel: [
      { vendor: "Elko", model: "EKO09738", description: "SmartStikk" },
    ],
    configure: async (device, coordinatorEndpoint) => {
  ...
  {
    fingerprint: [
      { modelID: "GreenPower_254", ieeeAddr: /^0x00000000e.......$/ },
    ],
    model: "A9MEM1570",
    vendor: "Schneider Electric",
    ...
    exposes: [
      ...
      e.power_apparent(),
      e
        .numeric("power_phase_a", ea.STATE)
        .withUnit("W")
        .withDescription("Instantaneous measured power on phase A"),
      e
        .numeric("power_phase_b", ea.STATE)
        .withUnit("W")
        .withDescription("Instantaneous measured power on phase B"),
      e
        .numeric("power_phase_c", ea.STATE)
        .withUnit("W")
        .withDescription("Instantaneous measured power on phase C"),
      e.power_factor(),
      e.energy(),
      e
        .numeric("energy_phase_a", ea.STATE)
        .withUnit("kWh")
        .withDescription("Sum of consumed energy on phase A"),
      e
        .numeric("energy_phase_b", ea.STATE)
        .withUnit("kWh")
        .withDescription("Sum of consumed energy on phase B"),
      e
        .numeric("energy_phase_c", ea.STATE)
        .withUnit("kWh")
        .withDescription("Sum of consumed energy on phase C"),
      e.ac_frequency(),
      e
        .numeric("voltage_phase_a", ea.STATE)
        .withUnit("V")
        .withDescription("Measured electrical potential value on phase A"),
      e
        .numeric("voltage_phase_b", ea.STATE)
        .withUnit("V")
        .withDescription("Measured electrical potential value on phase B"),
      e
        .numeric("voltage_phase_c", ea.STATE)
        .withUnit("V")
        .withDescription("Measured electrical potential value on phase C"),
      e
        .numeric("voltage_phase_ab", ea.STATE)
        .withUnit("V")
        .withDescription(
          "Measured electrical potential value between phase A and B"
        ),
      e
        .numeric("voltage_phase_bc", ea.STATE)
        .withUnit("V")
        .withDescription(
          "Measured electrical potential value between phase B and C"
        ),
      e
        .numeric("voltage_phase_ca", ea.STATE)
        .withUnit("V")
        .withDescription(
          "Measured electrical potential value between phase C and A"
        ),
      e
        .numeric("current_phase_a", ea.STATE)
        .withUnit("A")
        .withDescription(
          "Instantaneous measured electrical current on phase A"
        ),
      e
        .numeric("current_phase_b", ea.STATE)
        .withUnit("A")
        .withDescription(
          "Instantaneous measured electrical current on phase B"
        ),
      e
        .numeric("current_phase_c", ea.STATE)
        .withUnit("A")
        .withDescription(
    ...
    whiteLabel: [
      {
        ...
        model: "W599501",
        description: "Wiser smoke alarm",
        fingerprint: [{ modelID: "W599501" }],
      },
      {
        vendor: "Schneider Electric",
        model: "755WSA",
        description: "Clipsal Wiser smoke alarm",
        fingerprint: [{ modelID: "755WSA" }],
      },
    ],
  },
  {
  ...
  },
  {
    ...
    configure: async (device, coordinatorEndpoint) => {
      ...
      await reporting.thermostatOccupiedHeatingSetpoint(endpoint1);
      await reporting.bind(endpoint2, coordinatorEndpoint, ["seMetering"]);
      await endpoint1.read("hvacUserInterfaceCfg", [
        "keypadLockout",
        "tempDisplayMode",
      ]);
    },
  ...
  },
  {
    ...
    extend: [
      deviceEndpoints({
        endpoints: { left: 1, right: 2, left_sw: 21, right_sw: 22 },
      }),
      identify(),
