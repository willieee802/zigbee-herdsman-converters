...
const definitions: Definition[] = [
  ...
  },
  {
    ...
    fromZigbee: [
      fz.STS_PRS_251_presence,
      fz.battery,
      legacy.fz.STS_PRS_251_beeping,
    ],
    exposes: [
      e.battery(),
      e.presence(),
      e.action(["beeping"]),
      e
        .enum("beep", ea.SET, ["2", "5", "10", "15", "30"])
        .withDescription("Trigger beep for x seconds"),
    ],
  ...
  },
  {
    ...
    configure: async (device, coordinatorEndpoint) => {
      ...
      await reporting.temperature(endpoint);
      await reporting.batteryVoltage(endpoint);
      const payloadA = reporting.payload(
        "acceleration",
        10,
      ...
      const payloadX = reporting.payload(
        "x_axis",
        10,
        constants.repInterval.MINUTE,
        1
      ...
      const payloadY = reporting.payload(
        "y_axis",
        10,
        constants.repInterval.MINUTE,
        1
      ...
      const payloadZ = reporting.payload(
        "z_axis",
        10,
        constants.repInterval.MINUTE,
        1
  ...
  },
  {
    ...
    configure: async (device, coordinatorEndpoint) => {
      ...
      await endpoint.write(
        "manuSpecificSamsungAccelerometer",
        { 0x0000: { value: 0x01, type: 0x20 } },
        options
      );
      await endpoint.write(
        "manuSpecificSamsungAccelerometer",
        { 0x0002: { value: 0x0276, type: 0x21 } },
        options
      );
      await reporting.temperature(endpoint);
      await reporting.batteryVoltage(endpoint);
  ...
  },
  {
    ...
    exposes: [
      e.action(["off", "single", "double", "hold"]),
      e.battery(),
      e.temperature(),
    ],
    toZigbee: [],
