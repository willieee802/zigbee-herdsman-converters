  ...
  ctm_device_mode: {
    ...
    convert: (model, msg, publish, options, meta) => {
      ...
      if (data.hasOwnProperty(0x2200)) {
        const deviceModeLookup = {
          0: "astro_clock",
          1: "timer",
          2: "daily_timer",
          3: "weekly_timer",
        };
        result.device_mode = utils.getFromLookup(
          data[0x2200],
          deviceModeLookup
        );
      }
  ...
  ctm_thermostat: {
    ...
    convert: (model, msg, publish, options, meta) => {
      ...
      if (data.hasOwnProperty("elkoSensor")) {
        ...
        result.sensor = utils.getFromLookup(
          data["elkoSensor"],
          sensorModeLookup
        );
      }
      ...
      if (data.hasOwnProperty("elkoRegulatorMode")) {
        // Regulator mode
        result.regulator_mode = data["elkoRegulatorMode"]
          ? "regulator"
          : "thermostat";
      }
      ...
      if (data.hasOwnProperty("elkoExternalTemp")) {
        // External temp (floor)
        result.floor_temp =
          utils.precisionRound(data["elkoExternalTemp"], 2) / 100;
      }
      ...
      if (data.hasOwnProperty(0x0421)) {
        // Regulation mode
        const regulationModeLookup = {
          0: "thermostat",
          1: "regulator",
          2: "zzilent",
        };
        result.regulation_mode = utils.getFromLookup(
          data[0x0421],
          regulationModeLookup
        );
      }
  ...
  ctm_sove_guard: {
    ...
    convert: (model, msg, publish, options, meta) => {
      ...
      if (data.hasOwnProperty(0x0001)) {
        ...
        result.alarm_status = utils.getFromLookup(
          data[0x0001],
          alarmStatusLookup
        );
      }
      ...
      if (data.hasOwnProperty(0x0008)) {
        ...
        result.reset_reason = utils.getFromLookup(
          data[0x0008],
          resetReasonLookup
        );
      }
      ...
      if (data.hasOwnProperty(0x0100)) {
        ...
        result.current_flag = utils.getFromLookup(
          data[0x0100],
          currentFlagLookup
        );
      }
      ...
      if (data.hasOwnProperty(0x0102)) {
        // Relay status
        const relayStatusLookup = {
          0: "off",
          1: "on",
          2: "not_present",
          0xff: "unknown",
        };
        result.relay_status = utils.getFromLookup(
          data[0x0102],
          relayStatusLookup
        );
      }
      if (data.hasOwnProperty(0x0103)) {
        ...
        result.external_button = utils.getFromLookup(
          data[0x0103],
          relayStatusLookup
        );
      }
      if (data.hasOwnProperty(0x0104)) {
        ...
        result.relay_alarm = utils.getFromLookup(
          data[0x0104],
          relayAlarmLookup
        );
      }
      if (data.hasOwnProperty(0x0105)) {
        ...
        result.relay_alarm_status = utils.getFromLookup(
          data[0x0105],
          relayAlarmStatusLookup
        );
      }
...
const tzLocal = {
  ...
  ctm_device_enabled: {
    ...
    convertSet: async (entity, key, value, meta) => {
      await entity.write("genOnOff", {
        0x2201: {
          value: utils.getFromLookup(value, { OFF: 0, ON: 1 }),
  ...
  ctm_current_flag: {
    key: ["current_flag"],
    convertGet: async (entity, key, meta) => {
      await entity.read("genOnOff", [0x5000], {
  ...
  ctm_relay_state: {
    ...
    convertGet: async (entity, key, meta) => {
      await entity.read("genOnOff", [0x5001], {
        manufacturerCode: Zcl.ManufacturerCode.DATEK_WIRELESS_AS,
      });
    },
  } satisfies Tz.Converter,
      ...
      switch (key) {
        case "load":
          await entity.write("hvacThermostat", {
            0x0401: { value: value, type: Zcl.DataType.UINT16 },
          });
          break;
        case "display_text":
          await entity.write("hvacThermostat", {
            0x0402: { value: value, type: Zcl.DataType.CHAR_STR },
          });
          break;
        case "sensor":
          await entity.write("hvacThermostat", {
        ...
        case "system_mode":
          if (value === "off") {
            await entity.write("hvacThermostat", {
              0x0406: { value: 0, type: Zcl.DataType.BOOLEAN },
            });
          } else if (value === "heat") {
            await entity.write("hvacThermostat", {
              0x0422: { value: 3, type: Zcl.DataType.UINT8 },
            });
          }
          break;
        ...
        case "max_floor_temp":
          await entity.write("hvacThermostat", {
            0x0414: { value: value, type: Zcl.DataType.UINT8 },
          });
          break;
        case "regulator_setpoint":
          await entity.write("hvacThermostat", {
            0x0420: { value: value, type: Zcl.DataType.UINT8 },
          });
          break;
        ...
        case "exteral_sensor_source":
          await entity.write("hvacThermostat", {
            0x0428: { value: value, type: Zcl.DataType.UINT16 },
          });
          break;

        default: // Unknown key
          throw new Error(
            `Unhandled key tzLocal.ctm_thermostat.convertSet ${key}`
          );
      }
    },
    convertGet: async (entity, key, meta) => {
      switch (key) {
        ...
        default: // Unknown key
          throw new Error(
            `Unhandled key tzLocal.ctm_thermostat.convertGet ${key}`
          );
      }
    },
  } satisfies Tz.Converter,
  ctm_thermostat_preset: {
    ...
    convertSet: async (entity, key, value, meta) => {
      ...
      await entity.write("hvacThermostat", {
        0x0422: {
          value: utils.getFromLookup(value, presetLookup),
          type: Zcl.DataType.UINT8,
        },
      });
    },
  ...
  ctm_thermostat_child_lock: {
    ...
    convertSet: async (entity, key, value, meta) => {
      await entity.write("hvacThermostat", {
        0x0413: {
          value: utils.getFromLookup(value, { UNLOCK: 0, LOCK: 1 }),
          type: Zcl.DataType.BOOLEAN,
        },
      });
    },
  ...
  ctm_group_config: {
    key: ["group_id"],
    convertGet: async (entity, key, meta) => {
      await entity.read(0xfea7, [0x0000], {
        manufacturerCode: Zcl.ManufacturerCode.DATEK_WIRELESS_AS,
      });
    },
  } satisfies Tz.Converter,
  ctm_sove_guard: {
    ...
    convertGet: async (entity, key, meta) => {
      switch (key) {
        case "alarm_status":
          await entity.read(0xffc9, [0x0001], {
            manufacturerCode: Zcl.ManufacturerCode.DATEK_WIRELESS_AS,
          });
          break;
        case "battery_low":
          await entity.read(0xffc9, [0x0002], {
            manufacturerCode: Zcl.ManufacturerCode.DATEK_WIRELESS_AS,
          });
          break;
        case "stove_temperature":
          await entity.read(0xffc9, [0x0003], {
            manufacturerCode: Zcl.ManufacturerCode.DATEK_WIRELESS_AS,
          });
          break;
        case "ambient_temperature":
          await entity.read(0xffc9, [0x0004], {
            manufacturerCode: Zcl.ManufacturerCode.DATEK_WIRELESS_AS,
          });
          break;
        case "active":
          await entity.read(0xffc9, [0x0005], {
            manufacturerCode: Zcl.ManufacturerCode.DATEK_WIRELESS_AS,
          });
          break;
        case "runtime":
          await entity.read(0xffc9, [0x0006], {
            manufacturerCode: Zcl.ManufacturerCode.DATEK_WIRELESS_AS,
          });
          break;
        case "runtime_timeout":
          await entity.read(0xffc9, [0x0007], {
            manufacturerCode: Zcl.ManufacturerCode.DATEK_WIRELESS_AS,
          });
          break;
        case "reset_reason":
          await entity.read(0xffc9, [0x0008], {
            manufacturerCode: Zcl.ManufacturerCode.DATEK_WIRELESS_AS,
          });
          break;
        case "dip_switch":
          await entity.read(0xffc9, [0x0009], {
            manufacturerCode: Zcl.ManufacturerCode.DATEK_WIRELESS_AS,
          });
          break;
        case "sw_version":
          await entity.read(0xffc9, [0x000a], {
            manufacturerCode: Zcl.ManufacturerCode.DATEK_WIRELESS_AS,
          });
          break;
        case "hw_version":
          await entity.read(0xffc9, [0x000b], {
            manufacturerCode: Zcl.ManufacturerCode.DATEK_WIRELESS_AS,
          });
          break;
        case "bootloader_version":
          await entity.read(0xffc9, [0x000c], {
            manufacturerCode: Zcl.ManufacturerCode.DATEK_WIRELESS_AS,
          });
          break;
        case "model":
          await entity.read(0xffc9, [0x000d], {
            manufacturerCode: Zcl.ManufacturerCode.DATEK_WIRELESS_AS,
          });
          break;
        case "relay_address":
          await entity.read(0xffc9, [0x0010], {
            manufacturerCode: Zcl.ManufacturerCode.DATEK_WIRELESS_AS,
          });
          break;
        case "current_flag":
          await entity.read(0xffc9, [0x0100], {
            manufacturerCode: Zcl.ManufacturerCode.DATEK_WIRELESS_AS,
          });
          break;
        case "relay_current":
          await entity.read(0xffc9, [0x0101], {
            manufacturerCode: Zcl.ManufacturerCode.DATEK_WIRELESS_AS,
          });
          break;
        case "relay_status":
          await entity.read(0xffc9, [0x0102], {
            manufacturerCode: Zcl.ManufacturerCode.DATEK_WIRELESS_AS,
          });
          break;
        case "external_button":
          await entity.read(0xffc9, [0x0103], {
            manufacturerCode: Zcl.ManufacturerCode.DATEK_WIRELESS_AS,
          });
          break;
        case "relay_alarm":
          await entity.read(0xffc9, [0x0104], {
            manufacturerCode: Zcl.ManufacturerCode.DATEK_WIRELESS_AS,
          });
          break;
        case "relay_alarm_status":
          await entity.read(0xffc9, [0x0105], {
            manufacturerCode: Zcl.ManufacturerCode.DATEK_WIRELESS_AS,
          });
          break;
        ...
        default: // Unknown key
          throw new Error(
            `Unhandled key tzLocal.ctm_sove_guard.convertGet ${key}`
          );
      }
    },
  } satisfies Tz.Converter,
...
const definitions: Definition[] = [
  {
    ...
    configure: async (device, coordinatorEndpoint) => {
      ...
      await endpoint.read("lightingBallastCfg", [
        "minLevel",
        "maxLevel",
        "powerOnLevel",
      ]);
    ...
    whiteLabel: [
      {
        ...
        fingerprint: [{ modelID: "DimmerPille" }],
      },
    ],
  },
  {
    ...
    configure: async (device, coordinatorEndpoint) => {
      ...
      await endpoint.read(0xfea7, [0x0000], {
        manufacturerCode: Zcl.ManufacturerCode.DATEK_WIRELESS_AS,
      });
    },
    exposes: [
  ...
  },
  {
    ...
    exposes: [
      ...
      e
        ...
        .withUnit("W")
        .withDescription(
          "Load in W when heating is on (between 0-3600 W). The thermostat uses the value as input to the " +
            "mean_power calculation."
        )
        .withValueMin(0)
        .withValueMax(3600),
      e
        .text("display_text", ea.ALL)
        .withDescription(
          "Displayed text on thermostat display (zone). Max 19 characters"
        ),
      e
        .binary("regulator_mode", ea.ALL, "regulator", "thermostat")
        .withDescription("Device in regulator or thermostat mode."),
      e
        .numeric("mean_power", ea.STATE_GET)
        .withUnit("W")
        .withDescription("Reports average power usage last 10 minutes"),
      e
        .numeric("floor_temp", ea.STATE_GET)
        .withUnit("°C")
  ...
  },
  {
    ...
    exposes: [
      ...
      e
        .binary("active", ea.STATE, true, false)
  ...
  },
  {
    ...
    exposes: [
      ...
      e
        .binary("device_enabled", ea.ALL, "ON", "OFF")
        .withDescription("Turn the device on or off"),
      e
        .binary("child_lock", ea.STATE, "locked", "unlocked")
  ...
  },
  {
    ...
    exposes: [
      ...
      e.water_leak(),
      e
        .binary("active_water_leak", ea.STATE, true, false)
  ...
  },
  {
    ...
    configure: async (device, coordinatorEndpoint) => {
      ...
      // Relay State
      await endpoint.read("genOnOff", [0x5001], {
        manufacturerCode: Zcl.ManufacturerCode.DATEK_WIRELESS_AS,
      });
      await endpoint.configureReporting(
        "genOnOff",
  ...
  },
  {
    ...
    toZigbee: [
      tzLocal.ctm_mbd_device_enabled,
      tzLocal.ctm_relay_state,
      tzLocal.ctm_mbd_brightness,
      tz.ballast_config,
    ],
    meta: { disableDefaultResponse: true },
    configure: async (device, coordinatorEndpoint) => {
