...
import { BuffaloZclDataType } from "@willieee802/zigbee-herdsman/dist/zspec/zcl/definition/enums";
...
const sonoffExtend = {
    ...
    const exposes = e
      ...
      .withFeature(
        e
          ...
          .withDescription("Enable/disable inching function.")
      )
      .withFeature(
        e
      ...
      .withFeature(
        e
          .binary("inching_mode", ea.SET, "ON", "OFF")
          .withDescription("Set inching off or inching on mode.")
          .withValueToggle("ON")
      );
    ...
    const toZigbee: Tz.Converter[] = [
      {
        ...
        convertSet: async (entity, key, value, meta) => {
          ...
          const tmpTime = Number(
            Math.round(
              Number((value[inchingTime as keyof typeof value] * 2).toFixed(1))
            ).toFixed(1)
          );
          ...
          await entity.command(
            ...
            {
              manufacturerCode:
                Zcl.ManufacturerCode.SHENZHEN_COOLKIT_TECHNOLOGY_CO_LTD,
            }
          );
    ...
    const fromZigbee: Fz.Converter[] = [
      {
        ...
        convert: (model, msg, publish, options, meta) => {
          const day = Object.entries(constants.thermostatDayOfWeek).find(
            (d) => msg.data.dayofweek & (1 << +d[0])
          )[1];
    ...
    const toZigbee: Tz.Converter[] = [
      {
        ...
        convertSet: async (entity, key, value, meta) => {
          ...
          const transitionRegex =
            /^(0[0-9]|1[0-9]|2[0-3]):([0-5][0-9])\/(\d+(\.5)?)$/;

          utils.assertObject(value, key);

          for (const dayOfWeekName of Object.keys(value)) {
            const dayKey = utils.getKey(
              constants.thermostatDayOfWeek,
              dayOfWeekName.toLowerCase(),
              null
            );

            if (dayKey === null) {
              throw new Error(
                `Invalid schedule: invalid day name, found: ${dayOfWeekName}`
              );
            }
            ...
            if (transitions.length > 6) {
              throw new Error(
                "Invalid schedule: days must have no more than 6 transitions"
              );
            }
            ...
            for (const transition of transitions) {
              ...
              if (!matches) {
                throw new Error(
                  "Invalid schedule: transitions must be in format HH:mm/temperature (e.g. 12:00/15.5), " +
                    "found: " +
                    transition
                );
              }
              ...
              if (temp < 4 || temp > 35) {
                throw new Error(
                  `Invalid schedule: temperature value must be between 4-35 (inclusive), found: ${temp}`
                );
              }
            ...
            if (payload.transitions[0].transitionTime !== 0) {
              throw new Error(
                "Invalid schedule: the first transition of each day should start at 00:00"
              );
            }
  ...
  cyclicTimedIrrigation: (): ModernExtend => {
    const exposes = e
      ...
      .withFeature(
        e
          .numeric("current_count", ea.STATE)
          .withDescription("Number of times it has been executed")
          .withUnit("times")
      )
      .withFeature(
        e
    ...
    const fromZigbee: Fz.Converter[] = [
      {
        ...
        convert: (model, msg, publish, options, meta) => {
          ...
          if (attributeKey in msg.data) {
            ...
            const irrigationDurationBuffer =
              (buffer[2] << 24) |
              (buffer[3] << 16) |
              (buffer[4] << 8) |
              buffer[5];

            const irrigationIntervalBuffer =
              (buffer[6] << 24) |
              (buffer[7] << 16) |
              (buffer[8] << 8) |
              buffer[9];
    ...
    const toZigbee: Tz.Converter[] = [
      {
        ...
        convertSet: async (entity, key, value, meta) => {
          ...
          payloadValue[3] =
            value[irrigationDuration as keyof typeof value] >> 24;
          payloadValue[4] =
            value[irrigationDuration as keyof typeof value] >> 16;
          payloadValue[5] =
            value[irrigationDuration as keyof typeof value] >> 8;
          payloadValue[6] = value[irrigationDuration as keyof typeof value];

          payloadValue[7] =
            value[irrigationInterval as keyof typeof value] >> 24;
          payloadValue[8] =
            value[irrigationInterval as keyof typeof value] >> 16;
          payloadValue[9] =
            value[irrigationInterval as keyof typeof value] >> 8;
    ...
    const fromZigbee: Fz.Converter[] = [
      {
        ...
        convert: (model, msg, publish, options, meta) => {
          ...
          if (attributeKey in msg.data) {
            ...
            const irrigationCapacityBuffer =
              (buffer[2] << 24) |
              (buffer[3] << 16) |
              (buffer[4] << 8) |
              buffer[5];
    ...
    const toZigbee: Tz.Converter[] = [
      {
        ...
        convertSet: async (entity, key, value, meta) => {
          ...
          payloadValue[3] =
            value[irrigationCapacity as keyof typeof value] >> 24;
          payloadValue[4] =
            value[irrigationCapacity as keyof typeof value] >> 16;
          payloadValue[5] =
            value[irrigationCapacity as keyof typeof value] >> 8;
  ...
  externalTriggerMode: (): ModernExtend => {
    ...
    const exposes = e
      .enum("external_trigger_mode", ea.ALL, [
        "edge",
        "pulse",
        "following(off)",
        "following(on)",
    ...
    const fromZigbee: Fz.Converter[] = [
      {
        ...
        convert: (model, msg, publish, options, meta) => {
          ...
          logger.debug(
            `from zigbee msg.data['externalSwitchTriggerType'] ${msg.data["externalSwitchTriggerType"]}`,
            NS
          );
    ...
    const toZigbee: Tz.Converter[] = [
      {
        ...
        convertSet: async (entity, key, value, meta) => {
          ...
          await entity.write(clusterName, {
            [attributeName]: { value: tmpValue, type: Zcl.DataType.UINT8 },
          });
          return { state: { [key]: value } };
        },
        convertGet: async (entity, key, meta) => {
...
const definitions: Definition[] = [
  ...
  },
  {
    ...
    exposes: [
      e
        .numeric("light_indicator_level", ea.STATE)
        .withDescription("Brightness of the indicator light")
        .withAccess(ea.STATE),
    ],
    configure: async (device, coordinatorEndpoint) => {
      device.powerSource = "Mains (single phase)";
  ...
  },
  {
    ...
    extend: [
      ...
      numeric({
        ...
        description:
          "Minimum temperature at which to automatically turn on the radiator, " +
    ...
    configure: async (device, coordinatorEndpoint) => {
      ...
      await endpoint.read(
        0xfc11,
        [0x0000, 0x6000, 0x6002, 0x6003, 0x6004, 0x6005, 0x6006, 0x6007]
      );
    },
  },
  {
    ...
    extend: [
      ...
      enumLookup({
        ...
        lookup: {
          ...
          water_shortage: 1,
          water_leakage: 2,
          "water_shortage & water_leakage": 3,
        },
        cluster: 0xfc11,
  ...
  },
  {
    ...
    configure: async (device, coordinatorEndpoint) => {
      ...
      await endpoint.read(
        ...
        [0x0012, 0x0014, 0x0015, 0x0016, 0x0017]
      );
    },
  },
];
