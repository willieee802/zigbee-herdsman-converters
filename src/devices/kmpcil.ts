...
function handleKmpcilPresence(
  ...
  const useOptionsTimeoutBattery =
    options && options.hasOwnProperty("presence_timeout_battery");
  const timeoutBattery = useOptionsTimeoutBattery
    ? options.presence_timeout_battery
    : 420; // 100 seconds by default

  const useOptionsTimeoutDc =
    options && options.hasOwnProperty("presence_timeout_dc");
...
const kmpcilConverters = {
  ...
  presence_power: {
    ...
    type: ["attributeReport", "readResponse"],
    options: [
      kmpcilOptions.presence_timeout_dc(),
      kmpcilOptions.presence_timeout_battery(),
    ],
    convert: (model, msg, publish, options, meta) => {
      const payload = handleKmpcilPresence(model, msg, publish, options, meta);
...
const definitions: Definition[] = [
  {
    ...
    configure: async (device, coordinatorEndpoint) => {
      ...
      await endpoint.write(
        "genBinaryInput",
        { 0x0051: { value: 0x01, type: 0x10 } },
        options
      );
      await endpoint.write(
        "genBinaryInput",
        { 0x0101: { value: 25, type: 0x23 } },
        options
      );
  ...
  },
  {
    ...
    fromZigbee: [
      kmpcilConverters.presence_binary_input,
      kmpcilConverters.presence_power,
      fz.temperature,
    ],
    exposes: [
      e.battery(),
