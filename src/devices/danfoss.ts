...
const definitions: Definition[] = [
  {
    ...
    whiteLabel: [
      { vendor: "Danfoss", model: "014G2463" },
      {
        ...
        fingerprint: [{ modelID: "TRV001" }, { modelID: "TRV003" }],
      },
      {
        vendor: "Popp",
        model: "701721",
        description: "Smart thermostat",
        fingerprint: [{ modelID: "eT093WRO" }, { modelID: "eT093WRG" }],
      },
    ],
    ...
    exposes: (device, options) => {
      const maxSetpoint = ["TRV001", "TRV003"].includes(device?.modelID)
        ? 32
        : 35;
      return [
        e.linkquality(),
        ...
        e
          .binary("viewing_direction", ea.ALL, true, false)
          .withDescription(
        ...
        e
          .enum("setpoint_change_source", ea.STATE, [
            "manual",
            "schedule",
            "externally",
          ])
        ...
        e
          .binary("window_open_feature", ea.ALL, true, false)
          .withDescription("Whether or not the window open feature is enabled"),
        e
          .enum("window_open_internal", ea.STATE_GET, [
            "quarantine",
            "closed",
            "hold",
            "open",
        ...
        e
          ...
          .withValueMax(3600),
        e
          .numeric("load_estimate", ea.STATE_GET)
          .withDescription("Load estimate on this radiator")
          .withValueMin(-8000)
          .withValueMax(3600),
        e
          .binary("preheat_status", ea.STATE_GET, true, false)
          .withDescription(
            "Specific for pre-heat running in Zigbee Weekly Schedule mode"
          ),
        e
          .enum("adaptation_run_status", ea.STATE_GET, [
            "none",
            "in_progress",
            "found",
            "lost",
          ...
          .withDescription(
            "Status of adaptation run: None (before first run), In Progress, Valve Characteristic Found, " +
      ...
      ((endpointsCount) => {
        ...
        for (let i = 1; i <= endpointsCount; i++) {
          ...
          if (i != 16) {
            ...
            features.push(
              e
                .enum("output_status", ea.STATE_GET, ["inactive", "active"])
                .withEndpoint(epName)
                .withDescription("Danfoss Output Status [Active vs Inactive])")
            );
            features.push(
              e
            ...
            features.push(
              e
                ...
                .withEndpoint("l16")
                .withDescription("Regulator Status")
            );
            features.push(
              e
                .enum("system_status_water", ea.STATE_GET, [
                  "hot_water_flow_in_pipes",
                  "cool_water_flow_in_pipes",
                ])
                .withEndpoint("l16")
                .withDescription("Water Status of Regulator")
            );
            features.push(
              e
                .enum("multimaster_role", ea.STATE_GET, [
                  "invalid_unused",
                  "master",
                  "slave_1",
                  "slave_2",
                ...
                .withEndpoint("l16")
                .withDescription("Regulator role (Master vs Slave)")
            );
          }
        }
    ...
    configure: async (device, coordinatorEndpoint) => {
      ...
      for (let i = 1; i <= 15; i++) {
        ...
        if (typeof endpoint !== "undefined") {
          ...
          await endpoint.read(
            "hvacThermostat",
            ["danfossOutputStatus", "danfossRoomStatusCode"],
            options
          );
  ...
  },
  {
      ...
      ((endpointsCount) => {
        ...
        for (let i = 1; i <= endpointsCount; i++) {
          if (i < 16) {
            ...
            features.push(
              e
                .enum("room_floor_sensor_mode", ea.STATE_GET, [
                  "comfort",
                  "floor_only",
                  "dual_mode",
                ])
            ...
            features.push(
              e
                .numeric("temperature", ea.STATE_GET)
                .withUnit("°C")
                .withEndpoint(epName)
    ...
    configure: async (device, coordinatorEndpoint) => {
      ...
      if (typeof mainController != "undefined") {
        await reporting.bind(mainController, coordinatorEndpoint, [
          "genBasic",
          "haDiagnostic",
        ]);
