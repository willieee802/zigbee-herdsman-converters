...
import * as zigbeeHerdsman from "@willieee802/zigbee-herdsman/dist";
import * as semver from "semver";

export const manufacturerOptions = {
  manufacturerCode: zigbeeHerdsman.Zcl.ManufacturerCode.IKEA_OF_SWEDEN,
};

const bulbOnEvent: OnEvent = async (
  type,
  data,
  device,
  options,
  ...
  if (type === "deviceAnnounce") {
    ...
    if (levelConfig?.on_level !== undefined) {
      ...
      let onLevel: number;
      if (
        typeof onLevelRaw === "string" &&
        onLevelRaw.toLowerCase() == "previous"
      ) {
        onLevel = 255;
      } else {
...
export function ikeaLight(
  args?: Omit<LightArgs, "colorTemp"> & {
    colorTemp?: true | { range: Range; viaColor: true };
  }
) {
  const colorTemp: { range: Range } = args?.colorTemp
    ? args.colorTemp === true
      ? { range: [250, 454] }
      : args.colorTemp
    : undefined;
  ...
  if (isObject(args?.colorTemp) && args.colorTemp.viaColor) {
    result.toZigbee = replaceInArray(
      result.toZigbee,
...
export function ikeaBattery(): ModernExtend {
  ...
  const defaultReporting: ReportingConfigWithoutAttribute = {
    min: "1_HOUR",
    max: "MAX",
    change: 10,
  };
  ...
  const configure: Configure[] = [
    async (device, coordinatorEndpoint, definition) => {
      ...
      const bindTarget =
        ...
        (version[0] == 2 && version[1] == 3 && version[2] >= 75)
          ? coordinatorEndpoint
...
export function ikeaAirPurifier(): ModernExtend {
  const exposes: Expose[] = [
    presets
      .fan()
      .withModes(["off", "auto", "1", "2", "3", "4", "5", "6", "7", "8", "9"]),
    presets
    ...
    presets
      .enum("air_quality", access.STATE_GET, [
        "excellent",
        "good",
        "moderate",
        "poor",
    ...
    presets
      ...
      .withCategory("config"),
    presets
      .binary("child_lock", access.ALL, true, false)
    ...
    presets
      ...
      .withDescription("Time the filter has been used in minutes")
      .withCategory("diagnostic"),
  ];
  ...
  const fromZigbee: Fz.Converter[] = [
    {
      ...
      convert: (model, msg, publish, options, meta) => {
        ...
        if (msg.data.hasOwnProperty("particulateMatter25Measurement")) {
          const pm25Property = postfixWithEndpointName(
            "pm25",
            msg,
            model,
            meta
          ...
          let airQuality;
          const airQualityProperty = postfixWithEndpointName(
            "air_quality",
            msg,
            model,
            meta
        ...
        if (msg.data.hasOwnProperty("filterRunTime")) {
          ...
          state["replace_filter"] =
            parseInt(msg.data["filterRunTime"]) >= 259200;
  ...
  const toZigbee: Tz.Converter[] = [
    {
      ...
      convertSet: async (entity, key, value, meta) => {
        ...
        return {
          state: { fan_mode: value, fan_state: value === "off" ? "OFF" : "ON" },
        };
      },
      convertGet: async (entity, key, meta) => {
    ...
    },
    {
      ...
      convertGet: async (entity, key, meta) => {
        await entity.read("manuSpecificIkeaAirPurifier", [
          "particulateMatter25Measurement",
        ]);
      },
    },
    {
  ...
  const configure: Configure[] = [
    async (device, coordinatorEndpoint, definition) => {
      ...
      await endpoint.read("manuSpecificIkeaAirPurifier", [
        "controlPanelLight",
        "childLock",
        "filterRunTime",
      ]);
...
export function ikeaConfigureGenPollCtrl(args?: {
  endpointId: number;
}): ModernExtend {
  ...
  const fromZigbee: Fz.Converter[] = [
    {
      ...
      convert: (model, msg, publish, options, meta) => {
        ...
        if (
          onlyWhenOnFlag &&
          (!options ||
            !options.hasOwnProperty("illuminance_below_threshold_check") ||
            options.illuminance_below_threshold_check) &&
...
export function tradfriRequestedBrightness(): ModernExtend {
  const exposes: Expose[] = [
    presets
      .numeric("requested_brightness_level", access.STATE)
      .withValueMin(76)
      .withValueMax(254)
      .withCategory("diagnostic"),
    presets
      .numeric("requested_brightness_percent", access.STATE)
      .withValueMin(30)
      .withValueMax(100)
      .withCategory("diagnostic"),
  ];
  ...
  const fromZigbee: Fz.Converter[] = [
    {
      ...
      convert: (model, msg, publish, options, meta) => {
        return {
          ...
          requested_brightness_percent: mapNumberRange(
            msg.data.level,
            0,
            254,
            0,
  ...
  const fromZigbee: Fz.Converter[] = [
    {
      ...
      convert: (model, msg, publish, options, meta) => {
        ...
        const arrowReleaseAgo =
          Date.now() - globalStore.getValue(msg.endpoint, "arrow_release", 0);
...
export function ikeaDotsClick(args: {
  ...
  const actions = args.endpointNames
    .map((b) =>
      Object.values(args.actionLookup).map((a) =>
        args.dotsPrefix ? `dots_${b}_${a}` : `${b}_${a}`
      )
    )
    .flat();
  ...
  const fromZigbee: Fz.Converter[] = [
    {
      ...
      convert: (model, msg, publish, options, meta) => {
        ...
        return {
          action: args.dotsPrefix
            ? `dots_${button}_${action}`
            : `${button}_${action}`,
        };
      },
    },
  ...
  const configure: Configure[] = [
    setupConfigureForBinding("tradfriButton", "output", args.endpointNames),
  ];

  return { exposes, fromZigbee, configure, isModernExtend: true };
}

export function ikeaArrowClick(args?: {
  styrbar?: boolean;
  bind?: boolean;
}): ModernExtend {
    ...
    },
    {
      ...
      convert: (model, msg, publish, options, meta) => {
        ...
        if (direction) {
          ...
          const result = {
            action: `arrow_${direction}_release`,
            duration,
...
export function ikeaMediaCommands(): ModernExtend {
  ...
  const configure: Configure[] = [
    setupConfigureForBinding("genLevelCtrl", "output"),
  ];
