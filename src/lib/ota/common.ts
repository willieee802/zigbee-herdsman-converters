...
export async function getFirmwareFile(image: KeyValueAny, logger: Logger) {
  ...
  logger.debug(
    `OTA: Try to read firmware image from local file '${urlOrName}'`
  );
...
export async function getOverrideIndexFile(urlOrName: string) {
  if (isValidUrl(urlOrName)) {
    ...
    if (!index) {
      throw new Error(
        `OTA: Error getting override index file from '${urlOrName}'`
      );
    }
...
export function parseImage(
  ...
  assert(
    Buffer.compare(header.otaUpgradeFileIdentifier, upgradeFileIdentifier) ===
      0,
    `Not an OTA file`
  );
...
function validateImageData(image: Ota.Image) {
  for (const element of image.elements) {
    ...
    if (data.readUInt32BE(0) === gblTagHeader) {
      ...
      if (
        (tag === eblTagHeader && data.readUInt16BE(6) === eblImageSignature) ||
        tag === eblTagEncHeader
      ) {
...
function validateSilabsEbl(data: Buffer) {
  ...
  while (position + 4 <= dataLength) {
    ...
    for (let position2 = position; position2 < dataLength; position2++) {
      assert(
        data.readUInt8(position2) === eblPadding,
  ...
  throw new Error(
    `OTA: Image is truncated, not long enough to contain a valid tag`
  );
}
...
function sendQueryNextImageResponse(
...
function imageNotify(endpoint: Zh.Endpoint) {
  return endpoint.commandResponse(
    "genOta",
    "imageNotify",
...
function getImageBlockResponsePayload(
  ...
  let dataSize = Math.min(
    maximumDataSize,
    imageBlockRequest.payload.maximumDataSize
  );

  // Hack for https://github.com/Koenkk/zigbee-OTA/issues/328 (Legrand OTA not working)
  if (
    imageBlockRequest.payload.manufacturerCode === 4129 &&
    imageBlockRequest.payload.fileOffset === 50 &&
    imageBlockRequest.payload.maximumDataSize === 12
  ) {
    logger.info(
      `OTA: Detected Legrand firmware issue, attempting to reset the OTA stack`
    );
  ...
  logger.debug(
    `OTA: Payload offsets: start=${start} end=${end} dataSize=${dataSize}`
  );
...
function callOnProgress(
  ...
  if (lastUpdate === null || now - lastUpdate > 30000) {
    ...
    const remaining =
      (image.header.totalImageSize - imageBlockRequest.payload.fileOffset) /
      bytesPerSecond;
    let percentage =
      imageBlockRequest.payload.fileOffset / image.header.totalImageSize;
    percentage = Math.round(percentage * 10000) / 100;
    logger.debug(
      `OTA: Update at ${percentage}%, remaining ${remaining} seconds`
    );
...
export async function isUpdateAvailable(
  ...
  logger.debug(
    `OTA: Checking if an update is available for '${device.ieeeAddr}' (${device.modelID})`
  );
  ...
  if (requestPayload === null) {
    ...
    assert(
      endpoint != null,
      `Failed to find an endpoint which supports the OTA cluster`
    );
  ...
  const availableResult = await isNewImageAvailable(
    requestPayload,
    logger,
    device,
    getImageMeta
  ...
  logger.debug(
    `OTA: Update available for '${device.ieeeAddr}' (${device.modelID}): ${
      availableResult.available < 0 ? "YES" : "NO"
    }`
  );
  if (availableResult.available > 0) {
    logger.warn(
      `OTA: Firmware on '${device.ieeeAddr}' (${device.modelID}) is newer than latest firmware online.`
    );
  }
...
export async function isNewImageAvailable(
  ...
  logger.debug(
    `OTA: Is new image available for '${device.ieeeAddr}' (${device.modelID}), current '${currentS}'`
  );
  ...
  logger.debug(
    `OTA: Is new image available for '${device.ieeeAddr}' (${
      device.modelID
    }), latest meta '${JSON.stringify(meta)}'`
  );

  // Negative number means the new firmware is 'newer' than current one
  return {
    available: meta.force
      ? -1
      : Math.sign(current.fileVersion - meta.fileVersion),
    currentFileVersion: current.fileVersion,
...
export async function updateToLatest(
  ...
  logger.debug(
    `OTA: Updating to latest '${device.ieeeAddr}' (${device.modelID})`
  );
  const endpoint = getOTAEndpoint(device);
  assert(
    endpoint != null,
  ...
  logger.debug(
    `OTA: Got new image for '${device.ieeeAddr}' (${device.modelID})`
  );
  ...
  return new Promise((resolve, reject) => {
    const answerNextImageBlockOrPageRequest = () => {
      ...
      if (
        request.payload.manufacturerCode == 4742 &&
        request.payload.imageType == 8199
      ) {
      ...
      if (
        request.payload.manufacturerCode ==
      ...
      waiters.imageBlockOrPageRequest = {
        promise: Promise.race([
          imageBlockRequest.promise,
          imagePageRequest.promise,
        ]),
        cancel: () => {
      ...
      waiters.imageBlockOrPageRequest.promise.then(
        (imageBlockOrPageRequest) => {
          ...
          const sendImageBlockResponse = (
            imageBlockRequest: KeyValueAny,
            thenCallback: () => void,
            transactionSequenceNumber: number
          ) => {
            const payload = getImageBlockResponsePayload(
              image,
              imageBlockRequest,
              pageOffset,
              pageSize,
            ...
            const now = Date.now();
            const timeSinceLastImageBlockResponse =
              now - lastImageBlockResponse;

            // Reduce network congestion by only sending imageBlockResponse min every 250ms.
            const cooldownTime = Math.max(
              imageBlockResponseDelay - timeSinceLastImageBlockResponse,
              0
            );
            setTimeout(() => {
              endpoint
                ...
                .then(
                  ...
                  (e) => {
                    ...
                    thenCallback();
                    logger.debug(
                      `OTA: Image block response failed (${e.message})`
                    );
                  }
                );
            }, cooldownTime);

            lastUpdate = callOnProgress(
              startTime,
              lastUpdate,
              imageBlockRequest,
              image,
          ...
          if ("pageSize" in imageBlockOrPageRequest.payload) {
            ...
            const handleImagePageRequestBlocks = (
              imagePageRequest: KeyValueAny
            ) => {
              if (pageOffset < pageSize) {
                sendImageBlockResponse(
                  imagePageRequest,
                  () => handleImagePageRequestBlocks(imagePageRequest),
                  imagePageRequest.header.transactionSequenceNumber
                );
              } else {
        ...
        () => {
          ...
          reject(
            new Error(`OTA: Timeout, device did not request any image blocks`)
          );
        }
      );
    };
    ...
    const answerNextImageRequest = () => {
      waiters.nextImageRequest = endpoint.waitForCommand(
        "genOta",
        "queryNextImageRequest",
        null,
        maxTimeout
    ...
    waiters.upgradeEndRequest = endpoint.waitForCommand(
      "genOta",
      "upgradeEndRequest",
      null,
      maxTimeout
    ...
    waiters.upgradeEndRequest.promise.then((data) => {
      logger.debug(
        `OTA: Got upgrade end request for '${device.ieeeAddr}' (${
          device.modelID
      ...
      if (data.payload.status === 0) {
        ...
        endpoint
          .commandResponse(
            ...
            payload,
            null,
            data.header.transactionSequenceNumber
          )
          .then(
            () => {
              logger.debug(
                `OTA: Update succeeded, waiting for device announce`
              );
              onProgress(100, null);
              ...
              const cb = () => {
                logger.debug(
                  `OTA: Got device announce or timed out, call resolve`
                );
                clearInterval(timer);
        ...
        // @ts-expect-error
        const error = `OTA: Update failed with reason: '${
          endRequestCodeLookup[data.payload.status]
        }'`;
        logger.debug(error);
    ...
    imageNotify(endpoint).catch((e) =>
      logger.debug(`OTA: Image notify failed (${e})`)
    );
  });
}
...
export async function getNewImage(
  ...
  assert(
    meta,
    `Images for '${device.ieeeAddr}' (${device.modelID}) currently unavailable`
  );
  logger.debug(
    `OTA: Getting new image for '${device.ieeeAddr}' (${
      device.modelID
    }), latest meta ${JSON.stringify(meta)}`
  );
  assert(
    meta.fileVersion > current.fileVersion || meta.force,
    `No new image available`
  );

  const download = downloadImage
    ? await downloadImage(meta, logger)
  ...
  if (checksum) {
    ...
    logger.debug(
      `OTA: Update checksum validation succeeded for '${device.ieeeAddr}' (${device.modelID})`
    );
  }

  const start = download.data.indexOf(upgradeFileIdentifier);
  const image = parseImage(
    download.data.slice(start),
    suppressElementImageParseFailure
  );
  logger.debug(
    `OTA: Get new image for '${device.ieeeAddr}' (${
      device.modelID
    }), image header ${JSON.stringify(image.header)}`
  );
  assert(
    image.header.fileVersion === meta.fileVersion,
    `File version mismatch`
  );
  assert(
    !meta.fileSize || image.header.totalImageSize === meta.fileSize,
    `Image size mismatch`
  );
  assert(
    image.header.manufacturerCode === current.manufacturerCode,
    `Manufacturer code mismatch`
  );
  ...
  if (
    ...
    assert(
      image.header.minimumHardwareVersion <= device.hardwareVersion &&
        device.hardwareVersion <= image.header.maximumHardwareVersion,
    ...
    (error) => {
      ...
      if (error.response && [301, 302].includes(error.response.status)) {
        ...
        return axiosInstance.get(redirectUrl, {
          responseType: error.config.responseType || "arraybuffer",
        });
      }
    }
  );
