...
const optsLegrand = {
  identityEffect: () => {
    return e
      ...
      .withDescription(
        "Defines the identification effect to simplify the device identification."
      )
      .withFeature(
        e.enum("effect", ea.SET, Object.values(ledEffects)).withLabel("Effect")
      )
      .withFeature(
        e.enum("color", ea.SET, Object.values(ledColors)).withLabel("Color")
      );
  },
};

const getApplicableCalibrationModes = (
  isNLLVSwitch: boolean
): KeyValueString => {
...
export const _067776 = {
  getCover: (device: Zh.Device) => {
    ...
    const calMode = Number(
      device?.getEndpoint(1)?.clusters?.closuresWindowCovering?.attributes
        ?.calibrationMode
    );
    const showTilt = calMode
      ? shutterCalibrationModes[calMode]?.supportsTilt === true
      : false;
...
export const readInitialBatteryState: OnEvent = async (
  type,
  data,
  device,
  options
...
export const tzLegrand = {
  ...
  identify: {
    ...
    convertSet: async (entity, key, value, meta) => {
      ...
      const effectVariant = utils.getFromLookupByValue(
        selColor,
        ledColors,
        "0"
      );
    ...
    convert: (model, msg, publish, options, meta) => {
      ...
      if (msg.data.hasOwnProperty("0")) {
        ...
        if (option0 === 0x0001) payload.device_mode = "pilot_off";
          ...
          logger.warning(
            `Device_mode ${option0} not recognized, please fix me!`,
            NS
          );
