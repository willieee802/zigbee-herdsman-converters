...
function validateDefinition(definition: Definition) {
  ...
  assert.ok(
    Array.isArray(definition.exposes) ||
...
function processExtensions(definition: Definition): Definition {
  if ("extend" in definition) {
    if (!Array.isArray(definition.extend)) {
      assert.fail(
        `'${definition.model}' has legacy extend which is not supported anymore`
      );
    }
    ...
    const allExposesIsExposeOnly = (
      allExposes: (Expose | DefinitionExposesFunction)[]
    ): allExposes is Expose[] => {
    ...
    const addToAllExposes = (expose: Expose[] | DefinitionExposesFunction) => {
      typeof expose === "function"
        ? allExposes.push(expose)
        : allExposes.push(...expose);
    };
    ...
    const configures: Configure[] = definitionConfigure
      ? [definitionConfigure]
      : [];
    ...
    for (const ext of extend) {
      ...
      if (ext.ota) {
        if (ota && ext.ota !== ota) {
          assert.fail(
            `'${definition.model}' has multiple 'ota', this is not allowed`
          );
        }
        ota = ext.ota;
      }
      if (ext.endpoint) {
        if (endpoint) {
          assert.fail(
            `'${definition.model}' has multiple 'endpoint', this is not allowed`
          );
        }
    ...
    const actionExposes = allExposes.filter(
      (e) => typeof e !== "function" && e.name === "action"
    );
...
function prepareDefinition(definition: Definition): Definition {
  ...
  if (
    definition.exposes &&
    Array.isArray(definition.exposes) &&
    !definition.exposes.find((e) => e.name === "linkquality")
  ) {
    definition.exposes = definition.exposes.concat([
      exposesLib.presets.linkquality(),
    ]);
  }
  ...
  for (const expose of Array.isArray(definition.exposes)
    ? definition.exposes
    : definition.exposes(null, null)) {
    if (
      ...
      const type = utils.calibrateAndPrecisionRoundOptionsIsPercentual(
        expose.name
      )
        ? "percentual"
        : "absolute";
      definition.options.push(
        exposesLib.options.calibration(expose.name, type)
      );
      if (
        utils.calibrateAndPrecisionRoundOptionsDefaultPrecision[expose.name] !==
        0
      ) {
  ...
  for (const converter of [...definition.toZigbee, ...definition.fromZigbee]) {
    if (converter.options) {
      const options =
        typeof converter.options === "function"
          ? converter.options(definition)
          : converter.options;
...
export function postProcessConvertedFromZigbeeMessage(
  ...
  for (const [key, value] of Object.entries(payload)) {
    const definitionExposes = Array.isArray(definition.exposes)
      ? definition.exposes
      : definition.exposes(null, null);
    ...
    if (
      ...
      try {
        payload[key] = utils.calibrateAndPrecisionRoundOptions(
          value,
          options,
          expose.name
        );
        ...
        logger.logger.error(
          `Failed to apply calibration to '${expose.name}': ${error.message}`,
          NS
        );
      }
    }
...
export async function findByDevice(
  device: Zh.Device,
  generateForUnknown: boolean = false
) {
  let definition = await findDefinition(device, generateForUnknown);
  if (definition && definition.whiteLabel) {
    const match = definition.whiteLabel.find(
      (w) =>
        "fingerprint" in w &&
        w.fingerprint.find((f) => isFingerprintMatch(f, device))
    );
    if (match) {
      definition = {
        ...definition,
...
export async function findDefinition(
  ...
  if (!candidates) {
    ...
    const fingerprintMatch: { priority: number; definition: Definition } = {
      priority: null,
      definition: null,
    };
      ...
      if (candidate.fingerprint) {
        for (const fingerprint of candidate.fingerprint) {
          ...
          if (
            ...
            (!fingerprintMatch.definition ||
              priority > fingerprintMatch.priority)
          ) {
...
export async function generateExternalDefinitionSource(
  device: Zh.Device
): Promise<string> {
...
export function findByModel(model: string) {
  ...
  return definitions.find((definition) => {
    const whiteLabelMatch =
      definition.whiteLabel &&
      definition.whiteLabel.find((dd) => dd.model.toLowerCase() === model);
...
export async function onEvent(
  ...
  if (
    device.manufacturerID === Zcl.ManufacturerCode.LEGRAND_GROUP &&
    !device.customReadResponse
  ) {
    device.customReadResponse = (frame, endpoint) => {
      if (
        frame.isCluster("genBasic") &&
        frame.payload.find((i: { attrId: number }) => i.attrId === 61440)
      ) {
        const options = {
        ...
        endpoint
          ...
          .catch((e) => {
            logger.logger.warning(
              `Legrand security read response failed: ${e}`,
              NS
            );
          });
        return true;
    ...
    device.customReadResponse = (frame, endpoint) => {
      if (frame.isCluster("genTime")) {
        const oneJanuary2000 = new Date(
          "January 01, 2000 00:00:00 UTC+00:00"
        ).getTime();
        const secondsUTC = Math.round(
          (new Date().getTime() - oneJanuary2000) / 1000
        );
        ...
        endpoint
          ...
          .catch((e) => {
            logger.logger.warning(
              `ZNCWWSQ01LM custom time response failed: ${e}`,
              NS
            );
          });
        return true;
